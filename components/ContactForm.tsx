  ...
  return (
    <>
      ...
      <div style={{ textAlign: 'center', paddingTop: '0.5rem' }}>
        <Link href={`mailto:${userInfo.contact.email ? userInfo.contact.email.placeholder : ''}`}>{userInfo.contact.email.placeholder}</Link>
      </div>
      {
        userInfo.contact.phone ?
          <div className={styles.phone}>
            <Link href={`tel:${userInfo.contact.phone.countryCode} ${userInfo.contact.phone.placeholder}`}>{`${userInfo.contact.phone.countryCode} ${userInfo.contact.phone.placeholder}`}</Link>
          </div> : null
      }
